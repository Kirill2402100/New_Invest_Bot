...
def _fetch_td(pair: str, interval: str = "1min", bars: int = 2000) -> Optional[pd.DataFrame]:
    if not TD_KEY:
        log.warning("[TD] TWELVEDATA_API_KEY is empty — skipping TwelveData")
        return None
...
def _fetch_av(pair: str, interval: str = "1min", full: bool = True) -> Optional[pd.DataFrame]:
    ...
    if "Note" in j or "Information" in j:
        log.error(f"[AV] Rate limit / info: {j.get('Note') or j.get('Information')}")
        return None
...
def fetch_ohlcv(sym_or_pair: str, interval: str = "1min",
    ...
    """
    Унифицированный фид. Возвращает DataFrame с индексом-временем и колонками
    [open, high, low, close, volume].
    - sym_or_pair: 'EUR' -> EURUSD, 'USDJPY' -> USDJPY
    - interval: '1m','5m','15m','30m','1h','4h' (что возможно — TD),
                для AV будет сконвертировано в 1/5/15/30/60min
    - bars/limit: желаемое количество свечей (любой из параметров).
    """
    ...
    # 2) Alpha Vantage (интервалы AV — «min»)
