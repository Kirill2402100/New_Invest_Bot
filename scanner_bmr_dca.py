...
def render_remaining_levels_block(symbol: str, pos: "Position", bank: float,
    ...
    for i, t in enumerate(pos.ordinary_targets[base_off:], start=0):
        ...
        # состояние ПОСЛЕ гипотетического исполнения уровня
        qty_new = qty0 + dq
        ...
        # Equity и уровень маржи НА ЦЕНЕ добора
        ...
        lines.append(
            f"• {_display_label_ru_from_target(t)}: {fmt(price)} "
        ...
        # накопительно переносим состояние
        qty0, avg0, used0 = qty_new, avg_new, used_new
    return "\n".join(lines)

# --- HEDGE OPEN превью в требуемом формате ---
def render_hedge_preview_block(symbol: str, pos: "Position", bank: float,
                                fees_est: float, tick: float,
    ...
    L = max(1, int(getattr(pos, "leverage", 1) or 1))
    used0 = _pos_total_margin(pos)
    # состояние сразу после закрытия хеджа (PnL=0 на средней)
    ...
    # наращиваем суммарные лоты от стартовой ноги
    ...
    for i, t in enumerate(pos.ordinary_targets[base_off:], start=1):
        ...
        avg_new = (avg0 * qty0 + price * dq) / max(qty_new, 1e-9)
        used_new = used + m
        # временная позиция для оценки ПОСЛЕ шага
        class _Tmp: pass
        tmp = _Tmp()
        tmp.side = pos.side; tmp.avg = avg_new; tmp.qty = qty_new; tmp.leverage = L
        tmp.steps_filled = 1; tmp.step_margins = [used_new]; tmp.reserve_used = False; tmp.reserve_margin_usdt = 0.0
        # ML / equity на цене шага
        ml20 = ml_price_at(tmp, CONFIG.ML_TARGET_PCT, bank, fees_est)
        ...
        # строка блока для шага (в точной форме)
        lines.append(
...
def _equalize_p3_to_gap_and_ml(pos: "Position", bank: float, fees_est: float, tick: float) -> tuple[list[dict], bool]:
    ...
    if gap12 < CONFIG.DCA_MIN_GAP_TICKS:
        return pos.ordinary_targets, False
    # целевой p3 по равному разрыву к p2
    ...
    # применяем новые цены p1,p2,p3_target
    idxs = [i for i,t in enumerate(pos.ordinary_targets) if str(t.get("label","")).upper().startswith("STRAT")][:3]
...
def _fit_leg_with_equalization(
    ...
    """
    Подбирает leg_margin ≤ leg_margin_init, строит цели и равняет p3 так, чтобы
    gap(#2,#3) ≈ gap(#1,#2) и gap(#3,ML20%(после #3)) ≈ gap(#1,#2).
    """
    ...
    if best is not None:
        _, leg0, _, _, _ = best
        a = max(lo, leg0 * 0.6); b = min(hi, leg0 * 1.0)
        for _ in range(18):
            mid = (a + b) / 2.0
...
def clip_targets_by_ml(pos: "Position", bank: float, fees_est: float,
    ...
    if CONFIG.ML_REQ_GAP_MODE == "strat_spacing":
        # если уже известны STRAT и мы на n-м шаге, требуем запас >= разрыва между STRAT
    ...
    for i, t in enumerate(targets, start=0):
        ...
        used_new = used + m
        # --- NEW: резервный STRAT #3 не клиппуем; он исполняется только после reserve3_ready
        if t.get("reserve3"):
            out.append(t)
        ...
        try:
            ...
            if manual_dir in ("LONG", "SHORT"):
                ...
                if pos or _hedge_active():
                    ...
                    leg_sized, _pos, _targets, _fees_est = _fit_leg_with_equalization(
                    ...
                    _preview = render_hedge_preview_block(
                        symbol, _pos, alloc_bank_after, _fees_est, tick, planned_hc_px, lots_per_leg, leg_sized
                    )
                    await say(
            ...
            if _tac_req is not None:
                if not pos or not pos.from_hedge or b.get("fsm_state") != int(FSM.MANAGING):
                    ...
                    if v is None:
                        ...
                        # пересборка хвоста (TAC + STRAT1..3), клиппинг по ML, синхронизация резерва
                        ...
                        pos.ordinary_offset = min(getattr(pos, "ordinary_offset", 0), len(pos.ordinary_targets))
                        _sync_reserve3_flags(pos)
                        # сообщение в "прежнем формате": TAC/STRAT с доборами, ML и свободной маржей
                        ml_now = ml_price_at(pos, CONFIG.ML_TARGET_PCT, bank, fees_est)
                        ...
                        await say(
                            f"✏️ TAC обновлён вручную → <code>{fmt(tac_q)}</code>\n"
            ...
            if (b.get("hedge") and b["hedge"].get("active")):
                ...
                if _flip:
                    if not (b.get("hedge") and b["hedge"].get("active")):
                        ...
                        lots_leg = margin_to_lots(symbol, leg_margin, price=entry_px0, leverage=CONFIG.LEVERAGE)
                        _preview = render_hedge_preview_block(
                            symbol, _pos, alloc_bank_after, _fees, tick, planned_hc_px, lots_leg, leg_margin
                        )
                        await say(
            ...
            if (b.get("hedge") and b["hedge"].get("active") and b.get("hedge_close_price") is not None):
                ...
                lots_rem = margin_to_lots(symbol, pos.step_margins[0], price=entry_px, leverage=pos.leverage)
                ...
                await say(
                    ...
                    f"Оставлена нога: ≈{lots_rem:.2f} лота (маржа {pos.step_margins[0]:.0f} $).\n"
                    f"Средняя: <code>{fmt(pos.avg)}</code> (P/L 0) | TP: <code>{fmt(pos.tp_price)}</code>\n"
            ...
            if pos:
                if not pos or b.get("fsm_state") == int(FSM.IDLE) or pos.steps_filled <= 0:
                    ...
                    if (is_open_event or is_add_event) and (used_ord < pos.ord_levels) and allowed_now:
                        ...
                        levels_block = render_remaining_levels_block(symbol, pos, bank, CONFIG.FEE_TAKER, tick)  # Сообщение об исполнении шага / оформлении открытия
                        if is_add_event:
                        ...
                        # показываем только оставшиеся уровни после текущего добора
                        remaining = render_remaining_levels_block(symbol, pos, bank, fee_taker, tick)
                        msg_lines.append(remaining)
                        await say("\n".join(msg_lines))
