...
async def ticker(app: Application):
    """
    Отправляет статус сразу после запуска, а затем периодически:
    - каждые 30 мин в режиме FLAT
    - каждые 60 мин в режиме TREND
    Мгновенные уведомления о смене режима обрабатываются в `monitor`.
    """
    # Ждём, пока monitor впервые запишет актуальное значение ADX
    while state.get("last_adx_value") == 25.0:  # значение-по-умолчанию
        await asyncio.sleep(2)

    while True:
        try:
            # 1. Отправляем детальный статус (сразу при первом запуске)
            status_text = await get_status_text()
            await notify(status_text, app.bot)

            # 2. Определяем интервал для следующего отчёта на основе ТЕКУЩЕГО режима
            ...
            # 3. Ждём до следующего планового отчёта
